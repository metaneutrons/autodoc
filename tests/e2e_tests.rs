...
fn test_complete_workflow_init_to_build() {
    ...
    cmd.current_dir(temp_dir.path())
        .arg("build")
        .arg("html"); // HTML is most likely to work without XeLaTeX
    
    // Either succeeds or fails with proper error message
    let output = cmd.output().unwrap();
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(stderr.contains("Missing required dependencies") || stderr.contains("Pandoc failed"));
    }
}
...
fn test_diagram_workflow() {
    ...
    fs::write(temp_dir.path().join("diagram1.mmd"), "graph TD\n    A --> B").unwrap();
    fs::write(temp_dir.path().join("diagram2.mmd"), "sequenceDiagram\n    A->>B: Hello").unwrap();
...
fn test_full_project_lifecycle() {
    ...
    Command::cargo_bin("autodoc").unwrap()
    ...
    fs::write(temp_dir.path().join("custom.md"), r#"---
...
"#).unwrap();
    ...
    fs::write(temp_dir.path().join("flow.mmd"), "graph LR\n    Start --> End").unwrap();
